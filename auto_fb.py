...
def GetEmail():
    response = requests.post('your api').json()
...
def GetCode(email1):
    try:
        response = requests.get(f'https://api.int').text
...
def get_email_temp_mail():
    try:
        ...
        response = mail.post('https://api.int', data=payload, headers=headers)
...
def get_code_temp_plus(email):
    mail = requests.Session()
    headers = {
            /1220.1 Darwin/20.3.0',
            'Content-Type':'application/json',
            'Connection-type': 'wifi'
        }
    headers.update({'cookie': f'email={email}'})
    response = mail.get(f'httpls', headers=headers)
    print(response.json())
...
logo=("""
    </> Python Coded BY - AnonymousPro
    </> TELEGRAM - t.me/anonymousproofficial
""")
ll=str([hari,tanggal])
kk=str([xy,co])
hx=(f""" </> Python Coded BY - AnonymousPro """)
def banner():
    os.system("clear")
